...
class StudyActivity : AppCompatActivity() {
    ...
    private lateinit var progressBar: SpotsDialog
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //hooks
        progressBar =
            SpotsDialog.Builder().setContext(this).setCancelable(false).build() as SpotsDialog
        ...
        binding.btnUploadStudy.setOnClickListener {
            ...
            serialNo = binding.etLessonNoStudy.text.toString()
            lessonName = binding.etLessonTitleStudy.text.toString()
            source = binding.acCourseTeacherStudy.text.toString()
...
//        }
        ...
        if (category == "Notes") {
            fileName = "$lessonName _$time.pdf"

        } else {
            fileName = "${binding.etTitleAllStudy.text.toString()}_$time.pdf"
        }

        val db = FirebaseStorage.getInstance().reference
        val ref = db.child("Study")
            .child(year!!).child(courseCode!!)
            .child(fileName)
    ...
    private fun uploadToFirebase(fileUrl: String) {
        ...
        val ref = db.child("Study").child(year.toString())
            .child(courseCode.toString())
            .child(category.toString())
        //     study/ 1st Year/ Psy 201/ Notes/ 1

        val titleAll = binding.etTitleAllStudy.text.toString()
        val sourceAll = binding.tvSourceAllStudy.text.toString()

        //
        when {
            category.equals("Questions") -> {

                val quesMap: HashMap<String, Any> = HashMap()
                quesMap["lessonName"] = titleAll
                quesMap["source"] = "~ $sourceAll"
                quesMap["fileUrl"] = fileUrl

                ref.push().setValue(quesMap).addOnSuccessListener {
                    Toast.makeText(this, "Question upload successfully", Toast.LENGTH_SHORT).show()
                    progressBar.hide()
                }
                    .addOnFailureListener {
                        progressBar.hide()
                        Toast.makeText(this, "question :${it.message}", Toast.LENGTH_SHORT).show()
                    }
            }
            category.equals("Syllabus") -> {

                val sylMap: HashMap<String, Any> = HashMap()
                sylMap["lessonName"] = titleAll
                sylMap["source"] = "~ $sourceAll"
                sylMap["fileUrl"] = fileUrl

                ref.push().setValue(sylMap).addOnSuccessListener {
                    Toast.makeText(this, "Syllabus upload successfully", Toast.LENGTH_SHORT).show()
                    progressBar.hide()

                }.addOnFailureListener {
                    progressBar.hide()
                    Toast.makeText(this, "Syllabus :${it.message}", Toast.LENGTH_SHORT).show()
                }

            }
            category.equals("Books") -> {
                val booksMap: HashMap<String, Any> = HashMap()
                booksMap["lessonName"] = titleAll
                booksMap["source"] = "~ $sourceAll"
                booksMap["fileUrl"] = fileUrl

                ref.push().setValue(booksMap).addOnSuccessListener {
                    Toast.makeText(this, "Book upload successfully", Toast.LENGTH_SHORT).show()
                    progressBar.hide()

                }.addOnFailureListener {
                    progressBar.hide()
                    Toast.makeText(this, "Books :${it.message}", Toast.LENGTH_SHORT).show()
                }

            }
            else -> {

                val notesMap: HashMap<String, Any> = HashMap()
                notesMap["chapter"] = chapter.toString()
                notesMap["serialNo"] = serialNo.toString()
                notesMap["lessonName"] = lessonName.toString()
                notesMap["source"] = "~ ${source.toString()}"
                notesMap["fileUrl"] = fileUrl

                ref.child(chapter!!).push().setValue(notesMap).addOnSuccessListener {
                    Toast.makeText(this, "Notes upload successfully", Toast.LENGTH_SHORT)
                        .show()
                    progressBar.hide()
                }
                    .addOnFailureListener {
                        Toast.makeText(
                            this,
                            "Notes: ${it.message.toString()}",
                            Toast.LENGTH_SHORT
                        ).show()
                        Log.i("study", "notes upload failed: ${it.message.toString()}")
                        progressBar.hide()
                    }
        ...
        (binding.acCourseTopicStudy as AutoCompleteTextView?)?.onItemClickListener =
            AdapterView.OnItemClickListener { adapterView, view, position, l ->
                ...
                when (position) {
                    ...
                    0 -> {
                        binding.linearAllStudy.visibility = View.GONE
                    ...
                    1 -> {
                        ...
                        binding.linearAllStudy.visibility = View.VISIBLE
                    }
    ...
    private fun loadChapterNo1(code: String) {
        ...
        val chapter105 = listOf(
            "1Introduction",
            "2Data, Frequency Distribution and Graphical Representation",
        ...
        val chapter106 = listOf(
            "1Introduction of Computer",
            "2Operating System and Utility Programs",
            "3Introduction to Windows7",
            "4Transferring Files from Another Computer",
            "5Internet",
            "6Computer Security and safety"
        )
...
//            intent.type = "docx/*"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == PDF_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            // show file name
            val pathName = selectedFileUri?.lastPathSegment?.substringAfterLast("/")
            binding.tvDicNameStudy.text = "$pathName"

            binding.tvFileNameStudy.text = "One File selected"

        } else {
            Toast.makeText(applicationContext, "Please select a file", Toast.LENGTH_SHORT)
                .show()
        }
    }
